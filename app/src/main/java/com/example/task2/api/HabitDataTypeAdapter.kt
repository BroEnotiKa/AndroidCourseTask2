...
import com.example.task2.models.HabitPeriodicity
import com.example.task2.models.HabitPriority
...
class HabitDataTypeAdapter : TypeAdapter<HabitData>() {
    override fun write(writer: JsonWriter?, data: HabitData?) {
        data ?: return
        writer?.let { w ->
            w.beginObject()
            ...
            w.name("uid").value(data.id)
            w.endObject()
        }
    }

    override fun read(reader: JsonReader?): HabitData {
        var habitName = ""
        var description = ""
        var priority = 0
        var type = 0
